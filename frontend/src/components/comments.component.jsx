...
const CommentsContainer = () => {
    ...
    let { blog, blog: { _id, title, comments: commentsArr, activity: { total_parent_comments } }, commentsWrapper, setCommentsWrapper, totalParentCommentsLoaded, setTotalParentCommentsLoaded, setBlog } = useContext(BlogContext)
    ...
    const loadMoreComments = async () => {
        ...
        setBlog({ ...blog, comments: newCommentsArr.results })
    }
    ...
    return (
        <div className={"max-sm:w-full fixed " + (commentsWrapper ? "top-0 sm:right-0" : "top-[100%] sm:right-[-100%]") + " duration-700 max-sm:right-0 sm:top-0 w-[30%] min-w-[350px] h-full z-50 bg-white shadow-2xl p-8 px-16 overflow-y-auto overflow-x-hidden"}>
            ...
            <div className="relative">
                ...
                <button onClick={() => setCommentsWrapper(preVal => !preVal)} className="absolute top-0 right-0 flex justify-center items-center w-12 h-12 rounded-full bg-grey/20 hover:bg-grey/40">

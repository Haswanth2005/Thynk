...
const CommentCard = ({ index, leftVal, commentData }) => {
    ...
    let { blog, blog: { comments, activity, activity: { total_parent_comments }, author: { personal_info: { username: blog_author } } }, setBlog, setTotalParentCommentsLoaded } = useContext(BlogContext);
    ...
    const removeCommentsCards = (startingPoint, isDelete = false) => {
        if (comments[startingPoint]) {
            while (comments[startingPoint].childrenLevel > commentData.childrenLevel) {
                comments.splice(startingPoint, 1);
        ...
        if (isDelete) {
            ...
            if (parentIndex != undefined) {
                comments[parentIndex].children = comments[parentIndex].children.filter(child => child != _id)
        ...
        setBlog({ ...blog, comments: comments, activity: { ...activity, total_parent_comments: total_parent_comments - (commentData.childrenLevel == 0 && isDelete ? 1 : 0) } })
    }

    const loadReplies = ({ skip = 0, currentIndex = index }) => {

        if (comments[currentIndex].children.length) {

            hideReplies();

            axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/get-replies", { _id: comments[currentIndex]._id, skip })
                .then(({ data: { replies } }) => {

                    comments[currentIndex].isReplyLoaded = true;

                    for (let i = 0; i < replies.length; i++) {

                        replies[i].childrenLevel = comments[currentIndex].childrenLevel + 1;

                        comments.splice(currentIndex + 1 + i + skip, 0, replies[i])
                    }

                    setBlog({ ...blog, comments: [...comments] })

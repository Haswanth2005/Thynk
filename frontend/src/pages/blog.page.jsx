...
const BlogPage = () => {
  ...
  let { title, content, des, author: { personal_info: { fullname, username: author_username, profile_img } }, publishedAt } = blog
  ...
  const fetchBlog = () => {
    axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/get-blog", {
      ...
      .then(({ data: { blog } }) => {
        ...
        setBlog(blog)
        fetchComments({ blog_id: blog._id, setParentCommentCountFun: setTotalParentCommentsLoaded })
          .then(comments => {
            setBlog({ ...blog, comments: comments.results })
          })
        setLoading(false);
      })
      .catch(err => {
